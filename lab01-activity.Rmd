...
**Task** 
* Duplicate the last line in this code chunk and modify it by replacing the single `>` with `>>`. What happens when you rerun it? It duplicates the data in that document.

* What happens if you change the output file name (after `>>`) to `~/MBB243/out/genotype_head.txt`? 
The command does not work
Why do you think that is? Because the file does not exist.
...
#sort -f 4 ~/MBB243/genotype_head.txt
uniq -f 4 ~/MBB243/genotype_head.txt
```
...
Run the following code chunk once then use the RStudio terminal (not the Console) to find and view the contents of that file. If you have already "knit" the document then the code has been run. The `cat` command will dump the contents of any file to your terminal and is a convenient way to look at the contents of a file. For larger files, it's better to use either `head` to view the first few lines or `tail` to view the last few. What changes in the file if you manually run the code chunk a few more times? Everytime I run the code chuck it adds another line to the EMPTY.txt file
...
```{bash}
echo "source ~/.bashrc" >> ~/.bash_profile.txt
echo $(date) >> ~/.bash_profile.txt
```
`source ~/.bashrc`

The second line should be:
```{bash}
echo "DATA=/local-scratch/course_files/MBB243/" >> ~/.bash_profile.txt
echo $(date) >> ~/.bash_profile.txt
```
